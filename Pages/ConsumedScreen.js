...
const ConsumedScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Se vier da navegação com parâmetros
  ...
  // Salva os alimentos consumidos no AsyncStorage
  const saveConsumed = async (items) => {
    try {
      await AsyncStorage.setItem('@consumed', JSON.stringify(items));
    } catch (e) {
      console.error('Erro ao salvar alimentos consumidos', e);
    }
  };

  // Adiciona um alimento à lista de consumidos
  const addToConsumed = async (food) => {
    ...
    const updatedConsumed = [...consumed, foodWithTimestamp];
    setConsumed(updatedConsumed);
    await saveConsumed(updatedConsumed);
    Alert.alert('Sucesso', `${food.name} foi adicionado à sua lista de consumidos!`);
  };

  // Remove um alimento da lista de consumidos
  const removeFromConsumed = async (id) => {
    const updatedConsumed = consumed.filter(item => item.id !== id);
    setConsumed(updatedConsumed);
    await saveConsumed(updatedConsumed);
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ width: 25 }} /> // Espaço para alinhamento
      </View>
